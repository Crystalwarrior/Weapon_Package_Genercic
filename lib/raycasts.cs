...
function WeaponImage::onRaycastCollision(%this, %obj, %col, %pos, %normal, %vec)
{
	...
	{
		if (isObject(%this.raycastHitPlayerExplosion))
		{
			%data = %this.raycastHitPlayerExplosion;
		}
...
function Player::getLOSPoint(%this, %mask, %dist)
{
	...
	//%eyeVector = %this.getEyeVector();//%this.getMuzzleVector(0);
	//%fwdVector = %this.getForwardVector();
	//%eyeZ = getWord(%eyeVector,2);
	//%fwdXY = getWords(%fwdVector * (1 - mAbs(%eyeZ)),0,1);
	//%vector = %fwdXY SPC %eyeZ;
	%vector = %this.getEyeVector();
	%endPoint = vectorAdd(%eyePoint, vectorScale(%vector, %dist));
...
function createTracer(%a, %b, %dataBlock, %size, %color)
{
	...
	%obj.schedule(%datablock.tracerTime ? %dataBlock.tracerTime : 116, delete);
}