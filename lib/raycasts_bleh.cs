...
function WeaponImage::onRaycastCollision(%this, %obj, %col, %pos, %normal, %vec)
{
	if(!isObject(%col))
		return;
	if (%col.getType() & $TypeMasks::PlayerObjectType && isObject(%this.raycastHitPlayerExplosion))
	{
		%data = %this.raycastHitPlayerExplosion;
	}
	else if (isObject(%this.raycastHitExplosion))
	{
...
function Player::getLOSPoint(%this, %mask, %dist)
{
	...
	%eyeVector = %this.getEyeVector();//getWords(%this.getForwardVector(), 0, 1) SPC getWord(%this.getEyeVector(), 2);
